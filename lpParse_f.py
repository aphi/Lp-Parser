...
class Matrix:
    ...
    def __init__(self, parserObjective, parserConstraints, parserBounds, parserGenerals, parserBinaries):
        ...
        self.objective = Matrix.Objective(varExprToDict(parserObjective.varExpr), constants.objSenses[parserObjective.objSense], parserObjective.name)
        
        self.constraints = [Matrix.Constraint(varExprToDict(c.varExpr), constants.constraintSenses[c.sense], c.rhs, c.name) for c in parserConstraints]
...
def getBoundDict(parserBounds, parserBinaries):
    boundDict = defaultdict(lambda: {"lb": -constants.infinity, "ub": constants.infinity}) # need this versatility because the lb and ub can come in separate bound statements
